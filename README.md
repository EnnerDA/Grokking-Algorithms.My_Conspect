...
**MapReduce** - разновидность параллельных алгоритмов, распределяет задачи между множеством машин.

Функция map - применяет одну функцию к каждому элементу полученного массива.

Функция reduce - сокращает список элементов до одного элемента
```
>>> arrl = [1, 2, 3, 4, 5] 				
>>> reduce(lambda х,у: х+у, arrl)	
15
```
MapReduce использует эти две простые концепции для выполнения запросов на нескольких машинах.

**Фильтры Блума и HyperLogLog** - вероятностные алгоритмы.
...
Фильтры Блума очень удобны тогда, когда не нужно хранить точный ответ.
...
**Алгоритмы SHA (Secure Hash Algorithm)** - принимает строку и возвращает хэш-код этой строки. Для каждой строки будет уникальный хэш-код. При этом для одинаковых строк естественно хэш-код будет одинаковым.
Алгоритм помогает опеределить одинаковы ли файлы, используется при проверке паролей.

**Обмен ключами Дифии - Хеллмана** - алгоритм для шифровки информации. В основе лежит два ключа октрытый и закрытый. Один из лучших способов шифровать информацию которой обмениваются две стороны.
